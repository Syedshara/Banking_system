...
const Main = () => {
    ...
    const handleAcceptRequest = async (request, actionType) => {
        ...
        try {
            const response = await fetch('http://10.16.58.118:3000/lending_status', { // Replace with actual backend URL
                method: 'POST',
                headers: {
                        ...
                        borrowers.map((borrower) => (
                            <Card key={borrower.id} className="shadow-lg py-5 pt-5 h-80">
                                ...
                                <div className="flex justify-between">
                                    <Button color="green" onClick={() => handleAcceptRequest(borrower, 'accept')}>
                                        Accept
                                    </Button>
                                    <Button color="red" onClick={() => handleAcceptRequest(borrower, 'decline')}>
                                        Decline
                                    </Button>
                                </div>
                            </Card>
